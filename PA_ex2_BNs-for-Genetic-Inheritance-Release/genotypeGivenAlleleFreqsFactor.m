...
genotypeFactor.card = card_genotype;

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

for i = 1:genotypeFactor.card
    % fetch the correct allele frequences (i.e. probabilities)
    alleles = genotypesToAlleles(i,:);
    alleles_swapped = [alleles(2),alleles(1)];
    
    % in case gene contains one of each allele than because order does not
    % matter, we have weight this probability with factor 2
    if ~isequal(alleles,alleles_swapped)
        prob_i = 2 * prod(alleleFreqs(alleles));
    else
        prob_i = prod(alleleFreqs(alleles));
    end
    genotypeFactor.val(i) = prob_i;
end    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  