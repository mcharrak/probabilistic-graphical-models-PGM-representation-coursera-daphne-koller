...
for i = 1:numPeople
    ...
    % the following are indices equivalently enumerated with variables X1
    % to X9
    genotype_copy1_var = 0*numPeople +i;
    genotype_copy2_var = 1*numPeople +i;
    phenotype_var      = 2*numPeople +i;
        
    % step 1: GENOTYPES
    % fetch index of parents
    if (sum(pedigree.parents(genotype_copy1_var,:)) > 0)

        % A) genotype copy 1 variable

        parents_vars = pedigree.parents(genotype_copy1_var,:);
        % use parent 1 for child copy 1 genotype
        parent1_genotype_copy1_var = parents_vars(1);        
        parent1_genotype_copy2_var = parent1_genotype_copy1_var + numPeople;
        
        % generate factor with helper function
        factorList(genotype_copy1_var) = childCopyGivenParentalsFactor(numAlleles, ...
            genotype_copy1_var, parent1_genotype_copy1_var, parent1_genotype_copy2_var);
        
        % B) genotype copy 1 variable

        % use parent 2 for child copy 2 genotype
        parent2_genotype_copy1_var = parents_vars(2);        
        parent2_genotype_copy2_var = parent2_genotype_copy1_var + numPeople;        
               
        % generate factor with helper function
        factorList(genotype_copy2_var) = childCopyGivenParentalsFactor(numAlleles, ...
            genotype_copy2_var, parent2_genotype_copy1_var, parent2_genotype_copy2_var);        

        
    else
        % genotype copy 1 variable
        factorList(genotype_copy1_var) = childCopyGivenFreqsFactor(alleleFreqs,genotype_copy1_var);

        % genotype copy 2 variable
        factorList(genotype_copy2_var) = childCopyGivenFreqsFactor(alleleFreqs,genotype_copy2_var);        
    ...
    factorList(phenotype_var) = phenotypeGivenCopiesFactor(alphaList,...
        numAlleles, genotype_copy1_var, genotype_copy2_var, phenotype_var);
