...
for i = 1:numPeople
    ...
    genotype_var = i;
    phenotype_var = i+numPeople;
    
    % step 1: GENOTYPE
    % fetch index of parents
    if (sum(pedigree.parents(genotype_var,:)) > 0)
        parents_vars = pedigree.parents(genotype_var,:);
        genotypeVarChild = genotype_var;
        genotypeVarParentOne = parents_vars(1);
        genotypeVarParentTwo = parents_vars(2);
        % generate factor with helper function
        factorList(genotype_var) = genotypeGivenParentsGenotypesFactor(...
            numAlleles, genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo);
    else
        genotypeVar = genotype_var;
        factorList(genotype_var) = ...
            genotypeGivenAlleleFreqsFactor(alleleFreqs, genotypeVar);
        
    end
    
    % step 2: PHENOTYPE
    genotypeVar = genotype_var;
    phenotypeVar = phenotype_var;
    factorList(phenotype_var) = phenotypeGivenGenotypeFactor(...
